...
export default function Principal() {
    ...
    const botaoLogout = () => {
        ...
        setDadosUsuario(null); // Limpa os dados do usuário no context
        navigate("/");
        } catch (error) {
    ...
    return (
        <div className="flex h-screen font-sans bg-gradient-to-b from-purple-800 to-blue-700">
            <div className={`fixed inset-0 bg-red-500 bg-opacity-80 z-30 md:hidden ${menuAberto === true ? "block" : "hidden"}`}
                onClick={() => setMenuAberto(false)}>
            </div>

            {/* Sidebar */}
            <section className={`fixed top-0 left-0 w-64 h-full bg-gray-800 text-gray-200 flex flex-col z-40 transform transition-transform 
                md:relative md:w-20 lg:w-64 md:translate-x-0 ${menuAberto === true ? "translate-x-0" : "-translate-x-full"}`}>
                ...
                <div className="p-4 lg:p-6 border-t border-slate-700 bg-cyan-600 hover:bg-cyan-700 text-white font-bold py-2 rounded-lg m-4">
                        ...
                        <div className="border-t border-slate-700 mt-4 pt-4">
                            <div className="flex items-center p-2">
                                ...
                                <div className="ml-3 md:">
                                    ...
                                    <p className="text-xs text-gray-300">{dadosUsuario?.email}</p>
                                </div>
                            </div>
                            <button className="gap-2 flex items-center w-full justify-center p-3 rounded-lg duration-200 text-slate-300 font-bold" onClick={botaoLogout}>
            ...
            {/* Conteudo principal */}
                <section className="flex-1 p-4 text-gray-100 overflow-auto">
                    <header className="mb-3 flex items-center">
                        ...
                        <div className="flex-1 flex items-center gap-2 justify-center md:hidden">
                            <img src={logo} alt="Logo GFP" className="w-8 h-8" />
                            <span className="text-xl font-bold ">GFP</span>
                        </div>
